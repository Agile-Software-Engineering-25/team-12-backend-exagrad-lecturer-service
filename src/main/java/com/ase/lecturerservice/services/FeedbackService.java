...
public class FeedbackService {
  ...
  public void instantiateDummies() {
    ...
    DummyData.Feedbacks.forEach(feedbackRequest -> {this.saveFeedback(feedbackRequest, new MultipartFile[0]);});
  }
  ...
  public List<FeedbackResponse> getFeedbackForLecturer(String lecturerUuid) {
    ...
    return feedbacks.stream().filter(feedback ->
        Optional.ofNullable(getExam(feedback.getExamUuid()))
            ...
            log.info("feedback documents: {}", feedback.getFileReferences().toString());
            return feedbackMapper.toResponse(feedback, feedbackDocumentService.getDocumentsByFeedbackId(feedback.getUuid()));})
  ...
  public void saveFeedback(FeedbackRequest feedback, MultipartFile[] files) {
    ...
    if (files != null && files.length > 0) {
            ...
            try {
                FeedbackDocumentRequest metadata = new FeedbackDocumentRequest(
                    savedFeedback.getUuid(),
                    feedback.getLecturerUuid()
                );
                FeedbackDocument savedDocument = feedbackDocumentService.uploadFeedbackDocument(file, metadata);
        ...
        log.info("Uploaded {} files associated with feedback UUID: {}", files.length, savedFeedback.getUuid());
