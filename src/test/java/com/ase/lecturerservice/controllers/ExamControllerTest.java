...
public class ExamControllerTest {
  ...
  void fetchExamsShouldReturnExamDtos() throws Exception {
    ...
    mockMvc.perform(
            get("/api/v1/exams?lecturerUuid={lecturerUuid}", "Tom")
                .contentType(MediaType.APPLICATION_JSON))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(jsonPath("$[0].uuid").value(MockValues.UuidMocks.EXAM_UUID.getValue()))
  ...
  void fetchExamsShouldThrowException() throws Exception {
    ...
    mockMvc.perform(
            get("/api/v1/exams?lecturerUuid={lecturerUuid}", " ")
                .contentType(MediaType.APPLICATION_JSON))
